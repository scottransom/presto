  ...
  /*   same values for 'kern_half_width' and 'numbetween' as well.   */
{
  ...
  if (datainf == PREPPED || datainf == FFT || \
    ...
    if (datainf == PREPPED || datainf == FFT) fftlen = numdata;
    if (kerninf == PREPPED || kerninf == FFT) fftlen = numkern;
    if ((datainf == PREPPED || datainf == FFT) &&
	(kerninf == PREPPED || kerninf == FFT)){
      if (numdata != numkern){
  ...
  if (firsttime || !(datainf == SAME && \
	...
	for (ii = 0; ii < numbins; ii++) tmpdata[ii] = zeros;
  ...
  if (firsttime || !(kerninf == SAME && \
		     fftlen == oldfftlen)){

    if (!firsttime) free(kernarray);
  ...
  tmpdata = complex_corr_conv(dataarray, kernarray, fftlen, \
			      NOFFTS, optype);
  ...
  /*   'numpowers' is the number of complex points in 'powers'.       */
{
  ...
  if (firsttime || numpowers != old_numpowers) {
    ...
    for (ii = 0; ii < numpowers; ii++) kernel[ii] = 0.0;
