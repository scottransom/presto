...
int main(int argc, char *argv[])
{
  ...
  for (ii=0; ii<numread_old; ii++)
	  ...
	  realbuffer[ii] = inbuffer_old[ii].r * inbuffer_old[ii].r + inbuffer_old[ii].i * inbuffer_old[ii].i;
       }
  ...
  while ((numread_new=chkfread(inbuffer_new, sizeof(fcomplex), bufflen, infile)))
	  ...
	  for (ii=0; ii<numread_new; ii++)
	    {
	       ...
	       realbuffer[ii] = inbuffer_new[ii].r * inbuffer_new[ii].r + inbuffer_new[ii].i * inbuffer_new[ii].i;
	    }
	  ...
	  for (ii=0; ii<numread_old; ii++)
	    {
	       outbuffer[ii].r = 0.0;
	       outbuffer[ii].i = 0.0;
	       outbuffer[ii].r = inbuffer_old[ii].r/sqrt(mean_old+slope*((numread_old+numread_new)/2.0-ii));
	       outbuffer[ii].i = inbuffer_old[ii].i/sqrt(mean_old+slope*((numread_old+numread_new)/2.0-ii));
	    }
	  ...
	  printf("\rAmount Complete = %3.0f%%", 100*((binnum*2.0)/numsamp));
	  fflush(stdout);
  ...
  printf("\nDone.  Rednoise removed.  That is all, go home!\n\n");
