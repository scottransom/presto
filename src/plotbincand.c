...
int main(int argc, char *argv[])
{
  ...
  chkfileseek(candfile, (long) (candnum - 1), sizeof(binaryprops), \
	      SEEK_SET);
  ...
  data = read_fcomplex_file(fftfile, binprops.lowbin - lofreq, \
			    binprops.nfftbins);
  ...
  norm = sqrt((double) binprops.nfftbins * \
	      (double) numsumpow) / minifft[0].r;
  ...
  freqs = gen_freqs(binprops.nfftbins, 0.0, T / \
		    (2 * binprops.nfftbins));
  ...
  xyline(nzoom, freqs, minizoompow, "Binary Period (sec)", \
	 "Normalized Power", 1);
  free(freqs);
