...
struct spectra_info {
    ...
    double ra2000;          // RA  of observation (decimal deg, J2000)
    double dec2000;         // Dec of observation (decimal deg, J2000)
    double az;              // Azimuth (commanded) at the start of the obs (deg)
    double el;              // Elevation (commanded) at the start of the obs (deg)
    double MJDf;            // Fractional UTC MJD of starting epoch
    int MJDi;               // Integer UTC MJD of starting epoch
    ...
    int summed_pols;        // Are polarizations summed? (0=Yes, 1=No)
    int upper_sideband;     // Upper sideband?  (0=Yes, 1=No)
    int bits_per_sample;    // Number of bits per sample
    int bytes_per_spectra;  // Number of bytes in a spectra (inluding all polns)
    int spectra_per_file;   // Number of spectra per file
   ...
   if (allocd) {
      while (1) {
         ...
         if ((clip_sigma_st > 0.0) && !(mask && (*nummasked == -1)))
            memcpy(padvals, newpadvals, MAXCHAN);
