...
/* Get information for makedata from stdin.  */
{
  ...
  if (mdata->numonoff != 0) {
    ...
    if (tmponoff[i - 1] > 1.0) tmponoff[i - 1] = 1.0;
    mdata->numonoff = i/2;
...
/* Read the data for makedata from the makefile. */
{
  ...
  if (mdata->orb.p == 0.0 || mdata->orb.x == 0.0) mdata->binary = 0;
  else mdata->binary = 1;
  ...
  if (mdata->ampmoda == 0.0 || mdata->ampmodf == 0.0) mdata->ampmod = 0;
  else mdata->ampmod = 1;
