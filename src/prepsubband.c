	 ...
	 if ((datawrote == abs(*diffbinptr)) && (numwritten != numread) && (totwrote < cmd->numout)) {	/* Add/remove a bin */
	    ...
	    do {		/* Write the rest of the data after adding/removing a bin  */
		  ...
		  numtowrite = nextdiffbin;
	       if (cmd->subP)
		  write_subs(outfiles, cmd->nsub, subsdata, skip,
			     numtowrite);
	       else
		  write_data(outfiles, cmd->numdms, outdata, skip,
			     numtowrite);
	       numwritten += numtowrite;
	       datawrote += numtowrite;
	       totwrote += numtowrite;
			  ...
			  int numtowrite)
{
   ...
   if (numtowrite <= 0) {
	 ...
	 for (jj = 0; jj < numfiles; jj++)
	    chkfwrite(buffer, sizeof(float), numtowrite % veclen,
		      outfiles[jj]);
      }
      free(buffer);
   }
...
/* Read short int subband data written by prepsubband */
{
   ...
   for (ii = 0; ii < numfiles; ii++) {
      numread =
	  chkfread(subsdata, sizeof(short), SUBSBLOCKLEN, infiles[ii]);
