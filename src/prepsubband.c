...
int main(int argc, char *argv[])
{
  ...
  if (!RAWDATA){
	...
	cmd->wappP = 1;
      } else if (strcmp(suffix, "subs00")==0){
  ...
  if (!cmd->subsP){
    ...
    avgdm = cmd->subsdm;
    maxdm = cmd->subsdm;
    outfiles = (FILE **)malloc(cmd->numsub * sizeof(FILE *));
    for (ii=0; ii<cmd->numsub; ii++){
      sprintf(datafilenm, "%s_DM%.2f.subs%02d", cmd->outfile, avgdm, ii);
      outfiles[ii] = chkfopen(datafilenm, "wb");
      printf("   '%s'\n", datafilenm);
    }
  }
  ...
  if (cmd->nobaryP) { /* Main loop if we are not barycentering... */
    ...
    if (cmd->subsP)
      subsdata = gen_smatrix(cmd->numsub, worklen/cmd->downsamp);
    else
	...
	numtowrite = cmd->numout - totwrote;
      if (cmd->subsP)
	write_subs(outfiles, cmd->numsub, subsdata, 0, numtowrite);
      else 
    ...
    for (ii=0; ii<cmd->numdms; ii++){
      ...
      if (insubs){
	subdispdt = gen_dvect(cmd->numsub);
	for (ii=0; ii<cmd->numsub; ii++)
	  subdispdt[ii] = 0.0;
      } else {
	subdispdt = subband_delays(numchan, cmd->numsub, dms[ii], 
	...
	do { /* Write the rest of the data after adding/removing a bin  */
	  ...
	  if (cmd->subsP)
	    write_subs(outfiles, cmd->numsub, subsdata, skip, numtowrite);
	  else 
