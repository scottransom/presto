...
/* Reads the full pulsar database into the static array psrdata */
{
  ...
  sprintf(databasenm, "%s/pulsars.cat", path);
  ...
  for (ii=0; ii<np; ii++) {
	...
	if (pulsardata[ii].orb.p != 0.0) {
	  ...
	  for (jj=1, pmod=1.0; jj<41; jj++, pmod = 1.0/(double)jj) {
		...
		(4 * cand->ppsrerr)) {
	      for (kk=1, bmod = 1.0; kk<10; kk++, bmod=1.0/(double)kk) {
		if (fabs(pulsardata[ii].orb.p * bmod - cand->pbin / SECPERDAY) < \
		  ...
		  if (strlen(pulsardata[ii].bname)==0)
		    ...
		    sprintf(psrname, "B%s", pulsardata[ii].bname);
		  if (jj > 1) {
		    sprintf(tmp1, "Possibly the %s phasemod harmonic ", num[kk]);
		    if (full) {
		      sprintf(tmp2, "of the %s harmonic of PSR ", num[jj]);
	...
	if (pulsardata[ii].orb.p != 0.0) {
	  ...
	  for (jj=1; jj<41; jj++) {
	    ...
	    if (fabs(theop * pmod - cand->psr_p) < psrperr) {
	      for (kk=1; kk<10; kk++) {
		bmod = (double) kk;
		if (fabs(pulsardata[ii].orb.p * bmod - cand->orb_p / SECPERDAY) < orbperr) {
		  if (strlen(pulsardata[ii].bname)==0){
		    if (jj > 1) {
		      if (full) {
			sprintf(tmp1, "Possibly the %s phasemod harmonic ", num[kk]);
			sprintf(tmp2, "of the %s harmonic of PSR ", num[jj]);
			sprintf(tmp3, "J%s (p = %11.7f s, pbin = %9.4f d).\n", \
				pulsardata[ii].jname, theop, pulsardata[ii].orb.p);
			sprintf(output, "%s%s%s", tmp1, tmp2, tmp3);
		      } else {
			sprintf(output, "%s H J%.12s", num[kk], pulsardata[ii].jname);
		      }
		    } else {
		      if (full) {
			sprintf(tmp1, "Possibly the %s phasemod harmonic ", num[kk]);
			sprintf(tmp2, "of PSR J%s (p = %11.7f s, pbin = %9.4f d).\n", \
				pulsardata[ii].jname, theop, pulsardata[ii].orb.p);
			sprintf(output, "%s%s", tmp1, tmp2);
		      } else {
			sprintf(output, "PSR J%.12s", pulsardata[ii].jname);
		      }
		    }
		  } else {
		    if (jj > 1) {
		      if (full) {
			sprintf(tmp1, "Possibly the %s modulation harmonic ", num[kk]);
			sprintf(tmp2, "of the %s harmonic of PSR ", num[jj]);
			sprintf(tmp3, "B%s (p = %11.7f s, pbin = %9.4f d).\n", \
				pulsardata[ii].bname, theop, pulsardata[ii].orb.p);
			sprintf(output, "%s%s%s", tmp1, tmp2, tmp3);
		      } else {
			sprintf(output, "%s H B%s", num[kk], pulsardata[ii].bname);
		      }
		    } else {
		      if (full) {
			sprintf(tmp1, "Possibly the %s phasemod harmonic ", num[kk]);
