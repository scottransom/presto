...
extern void set_WAPP_static(int ptsperblk, int bytesperpt, int bytesperblk,
			    int numchan, int numifs, float clip_sigma,
			    double dt);
extern void get_GMRT_static(int *bytesperpt, int *bytesperblk,
			    float *clip_sigma);
...
int main(int argc, char *argv[])
{
   ...
   int numbarypts = 0, numread = 0, numtowrite = 0, totwrote =
       0, datawrote = 0;
   int padwrote = 0, padtowrite = 0, statnum = 0;
   int numdiffbins = 0, *diffbins = NULL, *diffbinptr =
       NULL, good_padvals = 0;
   double local_lodm;
   ...
   if (myid == 0) {		/* Master node only */
	 ...
	 if (split_root_suffix(cmd->argv[0], &root, &suffix) == 0) {
		...
		|| strcmp(suffix, "bcpm2") == 0) {
	       ...
	       cmd->bcpmP = 1;
		       ...
		       || strcmp(suffix, "fb") == 0) {
	       printf
		   ("Assuming the data is in SIGPROC filterbank format...\n");
	       cmd->filterbankP = 1;
	    } else if (strcmp(suffix, "fits") == 0 &&
		       strstr(root, "spigot_5") != NULL) {
	       printf
		   ("Assuming the data is from the NRAO/Caltech Spigot card...\n");
	       cmd->spigotP = 1;
	       ...
	       printf
		   ("Assuming the data is from the GMRT Phased Array system...\n");
	       cmd->gmrtP = 1;
	    } else if (isdigit(suffix[0]) &&
		       isdigit(suffix[1]) && isdigit(suffix[2])) {
	       printf
		   ("Assuming the data is from the Arecibo WAPP system...\n");
	       cmd->wappP = 1;
	 ...
	 if (numinfiles > 1)
	    ...
	    printf
		("Reading 1-bit filterbank (Parkes/Jodrell) data from 1 file:\n");
      } else if (cmd->bcpmP) {
	 if (numinfiles > 1)
   ...
   if (cmd->maskfileP) {
	 ...
	 good_padvals =
	     determine_padvals(cmd->maskfile, &obsmask, &padvals);
      }
...

   {
	 ...
	 if (cmd->wappP) {
	    ...
	    get_WAPP_file_info(infiles, cmd->numwapps, numinfiles,
			       ...
			       &N, &ptsperblk, &numchan, &dt, &T, &idata,
			       1);
	    get_WAPP_static(&bytesperpt, &bytesperblk, &numifs,
			    &clip_sigma);
   ...
   blocksperread = ((int)
		    (delay_from_dm
		     (cmd->lodm + cmd->numdms * cmd->dmstep, idata.freq)
		     / dsdt) / blocklen + 1);
   ...
   if (cmd->nobaryP) {		/* Main loop if we are not barycentering... */
	 ...
	 if (myid > 0) {
	    ...
	    if (!padding) {
		  ...
		  update_stats(statnum + ii, outdata[0][ii], &min,
			       &max, &avg, &var);
	       statnum += numtowrite;
	    }
	 }
   ...
   if (myid > 0) {
	 ...
	 if (!cmd->nobaryP) {
	    ...
	    barydispdt =
		delay_from_dm(dms[ii], doppler(baryhifreq, avgvoverc));
	 ...
	 for (ii = 0; ii < idata.numonoff; ii++) {
	    ...
	    for (jj = 0; jj < local_numdms; jj++)
	       chkfseek(outfiles[jj], (startpad + 1) * sizeof(float),
			SEEK_SET);
   ...
   while (1) {
	 ...
	 for (ii = 0; ii < blocksperread; ii++) {
	    if (myid == 0) {
	       if (cmd->pkmbP)
		  numread =
		      read_PKMB_rawblock(infiles, numfiles, &hdr,
					 rawdata, &tmppad);
	       else if (cmd->gmrtP)
		  numread =
		      read_GMRT_rawblock(infiles, numfiles, rawdata,
					 &tmppad);
	       else if (cmd->filterbankP)
		  numread =
		      read_filterbank_rawblock(infiles, numfiles,
					       rawdata, &tmppad);
	       else if (cmd->bcpmP)
		  numread =
		      read_BPP_rawblock(infiles, numfiles, rawdata,
					&tmppad);
	       else if (cmd->wappP)
		  numread =
		      read_WAPP_rawblock(infiles, numfiles, rawdata,
					 &tmppad, ifs);
	       else if (cmd->spigotP)
		  numread =
		      read_SPIGOT_rawblock(infiles, numfiles,
					   rawdata, &tmppad, ifs);
	       numread *= blocklen;
	    }
	    MPI_Bcast(&numread, 1, MPI_INT, 0, MPI_COMM_WORLD);
	    MPI_Bcast(&tmppad, 1, MPI_INT, 0, MPI_COMM_WORLD);
	    MPI_Bcast(rawdata, bytesperblk, MPI_UNSIGNED_CHAR, 0,
		      MPI_COMM_WORLD);
	    if (myid > 0) {
	       if (numread) {
		  if (cmd->pkmbP)
		  ...
		  *padding = 1;
		  for (jj = ii * blocksize; jj < (ii + 1) * blocksize;
		       jj++)
		     currentdata[jj] = 0.0;
	       }
	       if (tmppad)
