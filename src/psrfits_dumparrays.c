                      ...
                      float *data, int N, int *status) 
{
    ...
    fits_read_col(infile, TFLOAT, colnum, rownum, 1L, 
                  N, 0, data, &anynull, status);
    if (*status) {
...
int main(int argc, char *argv[]) {
    ...
    printf("#  Chan            Freq(MHz)         Weights         Scales          Offsets\n"
           "#-----------------------------------------------------------------------------\n");
    for (ii = 0 ; ii < npol ; ii++) {
