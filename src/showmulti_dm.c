...
int main(int argc, char *argv[])
{
  ...
  for (i = 0 ; i < numchan ; i++){
    ...
    if (rotate) drotate(&profs[i * proflen], proflen, rotate);
    for (j = 0 ; j < proflen ; j++){
  ...
  if (0 == strcmp("x", device)) cpgstart_x("portrait");
  else cpgstart_ps(output, "portrait");
  for (i = 0 ; i <= numchan/numtodisplay ; i++){
    offset = i * numtodisplay;
    numprofs = (numchan - offset) > numtodisplay ? \
      numtodisplay : numchan - offset;
    if (numprofs > 0){
      cpgpage();
