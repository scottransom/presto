...
#else
typedef struct fftw_plan{ int decoy; } fftw_plan;
 ...
 */
{
  ...
  while (bigfft_offset < max_offset) { /* Loop through the bigFFT */
    ...
    for (ii=0, jj=bigfft_offset; ii<worklen; ii++, jj++)
      powers[ii] = bigfft[jj].r * bigfft[jj].r + \
	bigfft[jj].i * bigfft[jj].i;
...
/* pulsations that have been phase-modulated.                    */
{
  ...
  for (ii=lobin; ii<hibin; ii++){
    pow = minifft[ii].r * minifft[ii].r +
      minifft[ii].i * minifft[ii].i;
    if (pow > threshold){
      (*numcands)++;
      cand_output(pow, ii, numminifft, 
		  lorfullfft, idata, *numcands);
    }
    interbin.r = twobypi * (minifft[ii].r - minifft[ii+1].r);
    interbin.i = twobypi * (minifft[ii].i - minifft[ii+1].i);
    pow = interbin.r * interbin.r +
      interbin.i * interbin.i;
    if (pow > threshold){
      (*numcands)++;
      cand_output(pow, ii+0.5, numminifft, 
		  lorfullfft, idata, *numcands);
    }
  }
}
