...
/* GET THE PARAMETERS */
   ...
   processargs(argc,argv, &chn_per_sample, &bins_input,
	       ...
	       &dm, &do_single_dm, &outtype, &bytesout_datatype,
               &dedisp_type);
...
/*  INITIALIZE DISPERSION DELAYS */
   ...
   if (dedisp_type > 1) {
     ...
     for(n=0;n<ndm;n++) {
	 dmvalues_tree[n] = n*sample_time*shift*frequency*frequency*frequency/8.3e3/(bandwidth/n_subbands);
   ...
   fclose(dmlist_tree);
	...
	while(dmvalues_tree[ndm_tree] < dm_opt) ndm_tree = ndm_tree + 1;
	if (ndm_tree < ndm) {
...
/* READ DATA INTO inp_buf: */
   ...
   /*if(quiet == 1) {
     ...
     if (loop != 0) bytes_inp = read_pipe(inp_buf, bts_inp_req);
    /*if(quiet == 1) {
	...
	nsave = 0;
	 ...
	 for (i=0; i<samples_out; i++) {
	   ...
	   ddata[ii][j] = (ddata[ii][j]-min[ns])/(max[ns]-min[ns])*255.;
           if (ddata[ii][j] < 0) ddata[ii][j] = 0;
           if (ddata[ii][j] > 255) ddata[ii][j] = 255;
	   ii=i+tkbk_samples;
	   if(outtype == 'c') {
...
*/
	...
	char *USAGE = "Usage:  dedisp  -b bins_input  -c chn_per_sample  -d ntimesamples -f frequency  -i invert   -m dm  -n n_subbands  -q  -s sec/sample  -t dedisp_type -u unflip  -w bandwidth  -z shift";
...
*/
	  ...
	  case 'o':
		   sscanf(optarg,"%c",outtype);
        	   if(*outtype == 'c') *bytesout_datatype = 1;
        	   if(*outtype == 's') *bytesout_datatype = 2;
        	   if(*outtype == 'i') *bytesout_datatype = 4;
        	   if(*outtype == 'f') *bytesout_datatype = 4;
		   break;
	  case 'q':
		   *quiet = 1; 
		   break; 
     ...
     fprintf(stderr," %i %i %f %i  %f %f %i %i %i %i %i %i %i \n",
	   ...
	   *frequency,*bandwidth,*invert,*quiet,*n_subbands,*ntimesamples,*unflip,
	   *dm,*do_single_dm);
	return;
