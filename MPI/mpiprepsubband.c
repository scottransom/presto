...
int main(int argc, char *argv[])
{
  ...
  int myid=0, numprocs=1, local_numdms;
  double local_lodm;
  char *datafilenm;
  PKMB_tapehdr hdr;
  infodata idata;
  ...
  local_numdms = cmd->numdms / numprocs;
  if (cmd->numdms & numprocs){
    if (myid==0){
  ...
  {
    double dt, T, clip_sigma=0.0;
    int ptsperblk, bytesperpt, bytesperblk=0, numifs=0;
    long long N;
    ...
    MPI_Bcast(&clip_sigma, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    blocklen = ptsperblk;
