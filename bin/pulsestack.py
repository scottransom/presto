...
if __name__=="__main__":
	...
	if opts.is_timeseries:            # plotting the time series
		...
		ax.plot (time, flux, color="%s" % (opts.color), marker="%s" % (opts.marker), markeredgecolor="%s" % (opts.markercolor), markerfacecolor="%s" % (opts.facecolor), linestyle="%s" % (opts.linestyle), linewidth=opts.linewidth, markeredgewidth=opts.markerwidth, markersize=opts.markersize, label="%s" % ("\n".join(opts.legend.split("\\n"))))
		# turn on grid
		...
		ax.plot (phase, flux, color="%s" % (opts.color), marker="%s" % (opts.marker), markeredgecolor="%s" % (opts.markercolor), markerfacecolor="%s" % (opts.facecolor), linestyle="%s" % (opts.linestyle), linewidth=opts.linewidth, markeredgewidth=opts.markerwidth, markersize=opts.markersize, label="%s" % ("\n".join(opts.legend.split("\\n"))))
		# turn on grid
		if opts.is_grid == True:
			plt.grid(True)
	...
	# Making the title
	if opts.title != '':
		ax.set_title("\n".join(opts.title.split("\\n")), fontsize=opts.fs, x=opts.titlepos.split(",")[0], y=opts.titlepos.split(",")[1], ha='%s' % (opts.titlepos.split(",")[-1]))

	# Putting the label
	if opts.label != '':
		ax.annotate("\n".join(opts.label.split("\\n")), fontsize=opts.fs, xycoords='axes fraction', xy=(0,0), xytext=(opts.labelpos.split(",")[0], opts.labelpos.split(",")[1]), ha='%s' % (opts.labelpos.split(",")[-1]))
