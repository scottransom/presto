...
if __name__ == "__main__":
    ...
    if DM != 0.0:
        if fold_pfd.nsub == 1:
            sys.stderr.write(
                "Warning: Do not set DM when using .pfds from de-dispersed time series!\n"
            )
        else:
            fold_pfd.bestdm = DM
    ...
    if not events:
        ...
        sumsubfreqs = (
            np.arange(numsubbands) + 0.5
        ) * subpersumsub * fold_pfd.subdeltafreq + (
        ...
        sumsubdelays = psr_utils.delay_from_DM(
            fold_pfd.bestdm, sumsubfreqs
        ) - psr_utils.delay_from_DM(fold_pfd.bestdm, fold_pfd.hifreq)
        sumsubdelays_phs = np.fmod(sumsubdelays / p_dedisp, 1.0)
        # Save the "higest channel within a subband" freqs/delays for use in
        # later DM/timing correction. PBD 2011/11/03
        sumsubfreqs_hi = (
            sumsubfreqs + fold_pfd.subdeltafreq / 2.0 - fold_pfd.chan_wid / 2.0
        )
        subdelays2 = psr_utils.delay_from_DM(
            fold_pfd.bestdm, sumsubfreqs
        ) - psr_utils.delay_from_DM(fold_pfd.bestdm, sumsubfreqs_hi)

    else:
        fold_pfd.subfreqs = np.asarray([0.0])
        sumsubfreqs = np.asarray([0.0])
        sumsubdelays = np.asarray([0.0])
        subdelays2 = np.asarray([0.0])
        sumsubdelays_phs = np.asarray([0.0])
    ...
    if fold.psr and fold.topo:
        if "polycos" in fold_pfd.__dict__ and not fold_pfd.polycos == 0:
    ...
    for ii in range(numtoas):
        ...
        if pcs is None:
            ...
            p = 1.0 / psr_utils.calc_freq(
                midtime, fold.epoch, fold.f0, fold.f1, fold.f2
            )
        ...
        for jj in range(numsubbands):
            ...
            if np.std(prof) == 0.0:
                sys.stderr.write(
                    "Skipping TOA %d for subband %d due to lack of signal\n"
                    % (ii + 1, jj + 1)
                )
                continue
            ...
            if not len(template) == fold_pfd.proflen:
                if not (
                    (len(template) % fold_pfd.proflen) == 0
                    or (fold_pfd.proflen % len(template)) == 0
                ):
                ...
                if len(template) > fold_pfd.proflen:
                    prof = psr_utils.linear_interpolate(
                        prof, len(template) // fold_pfd.proflen
                    )
                    if not ii and not jj:
                        sys.stderr.write(
            ...
            try:
                ...
                if len(prof) & 2 * len(prof):
                    ...
                    shift, eshift, snr, esnr, b, errb, ngood = measure_phase(
                        prof, template, rotate_prof, fftfit_code
                    )
                ...
                tau_tot = np.fmod(tau + sumsubdelays_phs[jj] + dd_phs_2 + 3.0, 1.0)
                if tau_tot > 0.5:
                    tau_tot -= 1.0
                ...
                if t2format:
                    flags = (
                        f"-ffphs {tau % 1:.4f} -fferr {tau_err:.4f}" if incphs else ""
                    )
                    psr_utils.write_tempo2_toa(
                        t0i + newdays,
                        toaf - newdays,
                        tau_err * p * 1000000.0,
                        sumsubfreqs[jj],
